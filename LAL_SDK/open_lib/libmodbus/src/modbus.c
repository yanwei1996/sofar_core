...
#include <modbus.h>
// #include "build/libmodbus-3.1.6/src/modbus.h"
#include <modbus-private.h>
// #include "build/libmodbus-3.1.6/src/modbus-private.h"
...
int modbus_send_raw_request(modbus_t *ctx, const uint8_t *raw_req, int raw_req_length)
{
    ...
    return send_msg(ctx, raw_req, raw_req_length);

    // printf(1, ("\n\n modbus data0 \n"));
    // for (i = 0; i < 16; i++)
	// {
    //     printf(1,("%02x ", raw_req[i]));
	// }
    // printf(1, ("\n\n"));
                                                  ...
                                                  msg_type_t msg_type)
{
    ...
    if (msg_type == MSG_INDICATION) {
        ...
        //dym add 
            ...
            length = 1;
        } else if(function == 0x43) {
            length = 6;
        } else if (function == 0x49) {
            length = 2;
        } else if (function == 0x50) {
            length = 1;
        } else if (function == 0x51) {
            length = 1;                                   
        } else if (function == 0x52) {
            length = 1;                                   
        } else {
   ...
   printf("\n compute_meta_length_after_function...: function=%02x length=%02x\n", function,length);
    return length;
}
                                          ...
                                          msg_type_t msg_type)
{
    ...
    if (msg_type == MSG_INDICATION) {
        ...
        case 0x41:
        case 0x42:
            if(msg[ctx->backend->header_length + 1] == 1)
            {
                length = 1;
            }
            else if(msg[ctx->backend->header_length + 1] == 2)
            {
                length = 8;
            }
            else
        ...
        case 0x50:
            ...
            else if(msg[ctx->backend->header_length + 1] == 3)
            {
                length = 6;
            }
            else
            ...
            break; 
        case 0x51:
            if(msg[ctx->backend->header_length + 1] == 2)
            {
                length = 2;
            }
            else if(msg[ctx->backend->header_length + 1] == 3)
            ...
            else if(msg[ctx->backend->header_length + 1] == 4)
            {
                length = 7;
            }
            else if(msg[ctx->backend->header_length + 1] == 5)
            {
                length = 0;
            }
            else
   ...
   printf("\n ctx->backend->header_length...:%02d\n", ctx->backend->header_length); 
   printf("\n compute_data_length_after_meta...:%02x\n", length);
//    length += ctx->backend->checksum_length;
    return length;
}

/* Computes the length to read after the meta information (address, count, etc) */
static int compute_data_length_after_define(modbus_t *ctx, uint8_t *msg,
                                          msg_type_t msg_type)
{
    int function = msg[ctx->backend->header_length];
    int length = 0;
    ...
    if (msg_type == MSG_INDICATION) {
        ...
        case 0x41:
            ...
            {
                length = (msg[ctx->backend->header_length + 8] << 8)|(msg[ctx->backend->header_length + 9]);
            }
            else
            {
                length = 0;
        ...
        case 0x50:
            ...
            {
                length = (msg[ctx->backend->header_length + 10] << 8)|(msg[ctx->backend->header_length + 11]);
            }
            else
            {
                length = 0;
    ...
    else if (msg_type == MSG_CONFIRMATION)  
    {
        switch (function) {
        case 0x42:
            if(msg[ctx->backend->header_length + 1] == 2)
            {
                length = (msg[ctx->backend->header_length + 6] << 8)|(msg[ctx->backend->header_length + 7]);
            }
            else
            {
                length = 0;
        ...
        case 0x51:
            if(msg[ctx->backend->header_length + 1] == 2)
            {
                if((msg[ctx->backend->header_length + 2] == 0) || (msg[ctx->backend->header_length + 2] == 1))
                {
                    length = 3;
                }
                else if(msg[ctx->backend->header_length + 2] > 1)
                {
                    length = 5;
                }
                else
    ...
    length += ctx->backend->checksum_length;
    printf("\n compute_data_length_after_define...: function : %02x %02x\n", function,length);
    return length;
}
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    {
        if(msg_length == 0) //第一次读取数据
        {
            rc = ctx->backend->select(ctx, &rset, p_tv, length_to_read);
        }
        else if(msg[ctx->backend->header_length] == 0x2B)
        {
            rc = ctx->backend->select(ctx, &rset, p_tv, 255);   //APOI:
        }
...
//        rc = ctx->backend->recv(ctx, msg + msg_length, length_to_read);       
        ...
        {
            rc = ctx->backend->recv(ctx, msg + msg_length, 255);    //APOI:
        }
        else
        {
            rc = ctx->backend->recv(ctx, msg + msg_length, length_to_read); //APOI:230306
        }
        ...
        if((msg[ctx->backend->header_length] == 0x2B) &&(msg_length != 0))
        {
            msg_length += rc;
            if (ctx->debug)
            {
                printf("line%d All msg_length %d \n", __LINE__,msg_length);
            }
        ...
        if (ctx->debug)
        {
            printf("\n line%d msg_length %02X ",__LINE__ ,msg_length);
            printf("\n length_to_read %02X \n", length_to_read);
        }
        ...
        if (length_to_read == 0) {
            ...
            case _STEP_META:
                ...
                if((msg[ctx->backend->header_length] == 0x41) || (msg[ctx->backend->header_length] == 0x42) || (msg[ctx->backend->header_length] == 0x51) 
                    || (msg[ctx->backend->header_length] == 0x50)  || (msg[ctx->backend->header_length] == 0x52))
                {
                    if(length_to_read != 0)
                    {
                        step = _STEP_DEFINE;
                ...
                break;
            case _STEP_DEFINE:
                length_to_read = compute_data_length_after_define(
                    ctx, msg, msg_type);
                if ((msg_length + length_to_read) > (int)ctx->backend->max_adu_length) {
                    errno = EMBBADDATA;
                    _error_print(ctx, "too many data");
...
int modbus_receive(modbus_t *ctx, uint8_t *req)
{
    if (ctx == NULL) {
        errno = EINVAL;
        	printf("modbus_receive ctx == NULL\n");
        return -1;
    }
...
{
    if (ctx == NULL ||
        (to_sec == 0 && to_usec == 0) || to_usec > 0xffffffff) {
        errno = EINVAL;
        return -1;
    }
