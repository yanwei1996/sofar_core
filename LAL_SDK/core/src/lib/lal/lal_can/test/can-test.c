//arm-linux-gnueabihf-gcc can-test.c -I../../../include -I/home/work/system/I.MX6ULL/ZLG-M6Y2C/buildroot-2020.02.1/output/staging/usr/include  --sysroot=/home/work/system/I.MX6ULL/ZLG-M6Y2C/buildroot-2020.02.1/output/staging/ -L../../ -l_sdk -lsqlite3 -o can-test
...
static int test_can_rw(int fd, int master)
{
    ...
    while (1) {
        ...
        FD_ZERO(&rset);
        FD_SET(fd, &rset);
        ret = select(fd+1, &rset, NULL, NULL, NULL);//NULL为阻塞
        if (ret == 0) {
...
int main(int argc, char *argv[])
{
    ...
    char order[100];
    memset(order, 0, 100);
    sprintf(order, "ifconfig %s down", argv[1]);
    system(order);
    memset(order, 0, 100);
    sprintf(order, "ip link set %s type can bitrate %d", argv[1], atoi(argv[2]));
    system(order);
    memset(order, 0, 100);
    sprintf(order, "ifconfig %s up", argv[1]);
    system(order);
